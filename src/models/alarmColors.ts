...
export function getAlarmColorName(alertClass: AlertClass, state: AlarmState): string | null {
    switch (state) {
        case 'came_unack':
            return alertClass.color_c_nack;
        case 'came_ack':
            return alertClass.color_c_ack;
        case 'went_unack':
            return alertClass.color_g_nack;
        default:
            return alertClass.color_none;
    }
}
...
export function getAlarmStateLabel(state: AlarmState): string {
    switch (state) {
        case 'came_unack':
            return 'CAME (unacknowledged)';
        case 'came_ack':
            return 'CAME (acknowledged)';
        case 'went_unack':
            return 'WENT (unacknowledged)';
        case 'went_ack':
            return 'WENT (acknowledged)';
        default:
            return '';
    }
}
